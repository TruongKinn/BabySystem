...
public class IntegrationService {
    ...
    private RestClient.RequestBodySpec injectSecurityHeaders(RestClient.RequestBodySpec spec, Long userId, String familyIdsHeader, boolean isAdmin) {
        if (userId != null) {
            spec = spec.header("X-User-Id", String.valueOf(userId));
        }
        if (org.springframework.util.StringUtils.hasText(familyIdsHeader)) {
            spec = spec.header("X-Family-Ids", familyIdsHeader);
        }
        spec = spec.header("X-User-Admin", String.valueOf(isAdmin));
        return spec;
    }

    public List<Map> getExpenseCategories(Long familyId, Long userId, String familyIdsHeader, boolean isAdmin) {
        try {
            ...
            Map response = requestSpec.retrieve().body(Map.class);
            if (response != null && Boolean.TRUE.equals(response.get("success"))) {
                return (List<Map>) response.get("data");
            }
        } catch (Exception e) {
            log.error("Failed to get expense categories for family " + familyId, e);
        }
        return List.of();
    }
    ...
    public BatchImportResponse importExpenses(BatchImportExpensesRequest request, Long userId, String familyIdsHeader, boolean isAdmin) {
        try {
            RestClient.RequestBodySpec requestSpec = restClientBuilder.build()
                    .post()
                    .uri(expenseServiceUri + "/api/expenses/batch")
    ...
    public BatchImportResponse importShopping(BatchImportShoppingRequest request, Long userId, String familyIdsHeader, boolean isAdmin) {
        int successCount = 0;
        int failedCount = 0;
        List<BatchImportResponse.RowError> errors = new ArrayList<>();
        ...
        try {
            ...
            listSpec = injectSecurityHeaders(listSpec, userId, familyIdsHeader, isAdmin);
            ...
            for (int i = 0; i < request.items().size(); i++) {
                ...
                try {
                    String quantityStr = item.quantity() != null ? String.valueOf(item.quantity().intValue()) : "1";
                    String notesStr = "Danh mục: " + item.category() + " - Đơn giá: " + (item.price() != null ? item.price().intValue() : 0);
                    if (org.springframework.util.StringUtils.hasText(item.notes())) {
                        notesStr += " - " + item.notes();
                    }
                    ...
                    itemSpec = injectSecurityHeaders(itemSpec, userId, familyIdsHeader, isAdmin);
                    Map<String, Object> itemResponse = itemSpec.retrieve().body(Map.class);
                    if (itemResponse != null && Boolean.TRUE.equals(itemResponse.get("success"))) {
                        successCount++;
                    } else {
    ...
    public BatchImportResponse importVaccinations(BatchImportVaccinationsRequest request, Long userId, String familyIdsHeader, boolean isAdmin) {
        int successCount = 0;
        int failedCount = 0;
        List<BatchImportResponse.RowError> errors = new ArrayList<>();
        ...
        for (int i = 0; i < request.vaccinations().size(); i++) {
            ...
            try {
                ...
                String notesStr = "Mũi số: " + vaccine.shotNo() + " - Chi phí tiêm: " + (vaccine.cost() != null ? vaccine.cost().intValue() : 0);
                if (org.springframework.util.StringUtils.hasText(vaccine.location())) {
                    notesStr += " - Nơi tiêm: " + vaccine.location();
                }
                if (org.springframework.util.StringUtils.hasText(vaccine.nextDateStr())) {
                    notesStr += " - Hẹn tiếp theo: " + vaccine.nextDateStr();
                }
    ...
    public BatchImportResponse importGrowthRecords(BatchImportGrowthRequest request, Long userId, String familyIdsHeader, boolean isAdmin) {
        int successCount = 0;
        int failedCount = 0;
        List<BatchImportResponse.RowError> errors = new ArrayList<>();
            ...
            try {
                ...
                if ((record.height() != null && record.height() > 0) || (record.weight() != null && record.weight() > 0)) {
                    ...
                    Map<String, Object> createGrowthBody = Map.of(
                            ...
                            "heightCm", height,
                            "headCircumferenceCm", BigDecimal.ZERO,
                            "notes", org.springframework.util.StringUtils.hasText(record.notes()) ? record.notes() : "Nhập từ file Excel"
                    );
                ...
                if (org.springframework.util.StringUtils.hasText(record.mealType()) || (record.intake() != null && record.intake() > 0)) {
                    BigDecimal intake = record.intake() != null ? BigDecimal.valueOf(record.intake()) : BigDecimal.ZERO;
                    
                    Map<String, Object> createLogBody = Map.of(
                            "logType", "FEEDING",
                            "value", intake,
                            "note", "Bữa ăn: " + record.mealType() + (org.springframework.util.StringUtils.hasText(record.notes()) ? " - " + record.notes() : ""),
                            "loggedAt", OffsetDateTime.now().toString()
                    );
