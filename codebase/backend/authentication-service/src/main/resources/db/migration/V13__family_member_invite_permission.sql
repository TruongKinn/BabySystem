...
SELECT 'API:POST:FAMILY_MEMBER_INVITE', 'Invite family member with login account', 'API', 'POST', '/account/families/{id}/members/invite'
WHERE NOT EXISTS (SELECT 1 FROM tbl_permission WHERE name = 'API:POST:FAMILY_MEMBER_INVITE');
...
WHERE p.name = 'API:POST:FAMILY_MEMBER_INVITE'
AND NOT EXISTS (
    SELECT 1
